...
export function PanelsView() {
  ...
  const [recoveryRevision,setRecoveryRevision]=useState<string>();
  ...
  const activate=async(id:string)=>{try{const row=artifacts.find(item=>item.id===id);await panelsApi(artifactActivationPath(artifactPanel!.id,id,row?.panel_version??artifactPanel!.version,row?.resource_manifest_id??recoveryRevision),{method:'POST'});message.success('Release activated');await Promise.all([openArtifacts(artifactPanel!),load()])}catch(reason){message.error((reason as Error).message)}};
  const activateResourceManifest=async(id:string)=>{try{await panelsApi(resourceActivationPath(artifactPanel!.id,id),{method:'POST'});message.success('Release activated');await Promise.all([openArtifacts(artifactPanel!),load()])}catch(reason){message.error((reason as Error).message)}};
  const syncFrontendManifest=async()=>{try{const result=await panelsApi<PanelRow>(`/panels/${artifactPanel!.id}/frontend-manifests/sync`,{method:'POST'});message.success(result.status==='STAGED'?'Artifact staged; publish its resource draft to activate':result.idempotent?'MF Manifest از قبل همگام بود':`MF Manifest همگام شد؛ route جدید: ${result.routesAdded??0}، حذف‌شده: ${result.routesRemoved??0}`);await Promise.all([openArtifacts(artifactPanel!),load()])}catch(reason){message.error((reason as Error).message)}};
  const fetchResourceManifest=async()=>{try{await panelsApi(`/panels/${artifactPanel!.id}/resource-manifests/fetch`,{method:'POST'});message.success('Manifest دریافت و به صورت Draft ثبت شد');await Promise.all([openArtifacts(artifactPanel!),load()])}catch(reason){message.error((reason as Error).message)}};
  ...
  >
    ...
    <Modal open={!!artifactPanel} title={`نسخه‌ها و Manifest — ${artifactPanel?.name_fa??''}`} onCancel={()=>setArtifactPanel(undefined)} footer={null} width={1000}>
      ...
      <Select allowClear value={recoveryRevision} onChange={setRecoveryRevision} placeholder="Resource revision for an unlinked historical artifact" style={{width:440,marginBottom:12}} options={resourceDrafts.filter(row=>row.workflowStatus==='PUBLISHED').map(row=>({value:row.id,label:row.manifestVersion}))}/>
