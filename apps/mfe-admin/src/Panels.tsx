...
export function PanelsView() {
  ...
  const [navigationOverrides,setNavigationOverrides]=useState<PanelRow[]>([]);
  ...
  const manifestSample=(row:PanelRow)=>JSON.stringify({schemaVersion:'1.0',module:{key:row.slug,name:row.name_en,nameFa:row.name_fa,nameEn:row.name_en,version:row.semantic_version},routes:[{key:'index',path:'/home',component:'./Home',resourceKey:`page:${row.slug}.home`,action:'view',title:row.name_fa}],resources:[{key:`page:${row.slug}.home`,type:'PAGE',name:'Home',nameFa:'صفحه اصلی',nameEn:'Home',actions:['view']}],navigation:[{key:`${row.slug}.nav.root`,type:'GROUP',title:row.name_fa,order:10},{key:`${row.slug}.nav.home`,type:'PAGE',parentKey:`${row.slug}.nav.root`,pageKey:'index',title:'صفحه اصلی',order:10}]},null,2);
  const openArtifacts=async(row:PanelRow)=>{setArtifactPanel(row);const[a,d,n]=await Promise.all([panelsApi<PanelRow[]>(`/panels/${row.id}/artifacts`),panelsApi<PanelRow[]>(`/panels/${row.id}/resource-manifests`),panelsApi<PanelRow[]>(`/panels/${row.id}/navigation-overrides`)]);setArtifacts(a);setResourceDrafts(d);setNavigationOverrides(n);setResourceManifest(manifestSample(row));artifactForm.setFieldsValue({artifactVersion:row.semantic_version,remoteEntryUrl:row.remote_entry_path,remoteName:row.remote_name,exposedModule:'./plugin',contractVersion:'1.0',manifest:JSON.stringify({schemaVersion:'1.0',moduleKey:row.slug,defaultRouteId:'index',routes:[{id:'index',path:'',title:row.name_fa,resource:`application:aurevia/${row.slug}`,action:'view'}],menus:[{id:'main',routeId:'index',title:row.name_fa,order:10}]},null,2)});navigationForm.setFieldsValue({source:'ADMIN',nodeType:'GROUP',hidden:false,order:50})};
  ...
  >
    ...
    <Modal open={!!artifactPanel} title={`نسخه‌ها و Manifest — ${artifactPanel?.name_fa??''}`} onCancel={()=>setArtifactPanel(undefined)} footer={null} width={1000}>
      ...
      <Card size="small" title="Navigation Overlay مستقل از Resource Tree" style={{marginTop:16}}>
        <Table rowKey="key" size="small" dataSource={navigationOverrides} pagination={false} columns={[{title:'Key',dataIndex:'key'},{title:'نوع',dataIndex:'nodeType'},{title:'مالک',dataIndex:'source'},{title:'عنوان',dataIndex:'title'},{title:'والد',dataIndex:'parentKey'},{title:'پنهان',dataIndex:'hidden',render:value=>value?'بله':'خیر'}]}/>
