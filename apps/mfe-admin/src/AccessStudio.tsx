...
const typeMeta: Record<string,ResourceMeta> & Record<(typeof resourceTypes)[number],ResourceMeta> = {
  ...
  API_RESOURCE:{label:'منبع API',color:'volcano',icon:'⌁',hint:'قابلیت API مستقل از URL و متد؛ route از طریق binding جداگانه متصل می‌شود'},
  DATA_RESOURCE:{label:'منبع داده',color:'lime',icon:'▱',hint:'مجموعه‌داده یا قلمرو داده با entitlement مستقل'},
  DATA_GOVERNANCE_RESOURCE:{label:'منبع حاکمیت داده',color:'orange',icon:'◇',hint:'سیاست یا قابلیت حاکمیتی مرتبط با منبع داده'},
};
...
export function AccessStudio(){
  ...
  const[showInactivePanelResources,setShowInactivePanelResources]=useState(false);
  ...
  const inactivePanelIds=useMemo(()=>new Set(panels.filter(panel=>!panel.active).map(panel=>panel.id)),[panels]);
  const activeResources=useMemo(()=>resources.filter(r=>r.status==='ACTIVE'&&(showInactivePanelResources||!r.panel_id||!inactivePanelIds.has(r.panel_id))),[resources,inactivePanelIds,showInactivePanelResources]);
  const hiddenInactivePanelResourceCount=useMemo(()=>resources.filter(r=>r.status==='ACTIVE'&&r.panel_id&&inactivePanelIds.has(r.panel_id)).length,[resources,inactivePanelIds]);
  const filteredIds=useMemo(()=>new Set(activeResources.filter(r=>(typeFilter==='ALL'||r.type===typeFilter)&&(!query||`${r.name_fa} ${r.name_en} ${r.resource_key}`.toLowerCase().includes(query.toLowerCase()))).map(r=>r.id)),[activeResources,query,typeFilter]);
  ...
  return <Space direction="vertical" size={16} style={{width:'100%'}}>
    ...
    <Row gutter={16} align="stretch">
      <Col xs={24} lg={10}><Card title="درخت Manifest" loading={loading} styles={{body:{minHeight:560}}} extra={<Tag color="green">Service-backed</Tag>}>
        <Space direction="vertical" style={{width:'100%'}} size={12}><Input.Search allowClear placeholder="جست‌وجوی نام یا کلید منبع…" onChange={e=>setQuery(e.target.value)}/><Select value={typeFilter} onChange={setTypeFilter} style={{width:'100%'}} options={[{value:'ALL',label:'همه انواع منابع'},...resourceTypes.map(type=>({value:type,label:`${typeMeta[type].icon} ${typeMeta[type].label}`}))]}/><Space><Switch size="small" checked={showInactivePanelResources} onChange={setShowInactivePanelResources}/><Typography.Text type="secondary">نمایش منابع پنل‌های غیرفعال{hiddenInactivePanelResourceCount>0?` (${hiddenInactivePanelResourceCount})`:''}</Typography.Text></Space>{tree.length?<Tree blockNode showLine defaultExpandAll treeData={tree} selectedKeys={selected?[selected.id]:[]} onSelect={keys=>setSelected(resources.find(r=>r.id===keys[0]))}/>:<Empty description="منبعی مطابق فیلتر پیدا نشد"/>}</Space>
      </Card></Col>
      <Col xs={24} lg={14}>{selected?<Space direction="vertical" size={16} style={{width:'100%'}}>
        <Card title={<Space><TypeTag type={selected.type}/><span>{selected.name_fa}</span><Tag color={selected.source==='MANIFEST'?'blue':'purple'}>{selected.source}</Tag></Space>} extra={<Space><Button disabled={selectedPanelMode==='MANIFEST'&&!developmentMutationsEnabled} onClick={()=>openEditor(undefined,selected)}>افزودن فرزند</Button><Button onClick={()=>openEditor(selected)}>{selected.source==='MANIFEST'&&!developmentMutationsEnabled?'تنظیم نمایش':'ویرایش منبع'}</Button>{(selected.source==='ADMIN'||developmentMutationsEnabled)&&<Popconfirm title="منبع از درخت فعال حذف و DEPRECATED شود؟" onConfirm={()=>void deprecate()}><Button danger>حذف از درخت</Button></Popconfirm>}</Space>}>
          <Descriptions size="small" column={{xs:1,md:2}} items={[{key:'key',label:'کلید canonical',children:<Typography.Text copyable code>{selected.resource_key}</Typography.Text>},{key:'owner',label:'دامنه مالک',children:selected.owner_domain||'—'},{key:'class',label:'طبقه‌بندی',children:<Tag>{selected.classification||'تعریف‌نشده'}</Tag>},{key:'status',label:'وضعیت',children:<Badge status={selected.status==='ACTIVE'?'success':'default'} text={selected.status}/>},{key:'manifest',label:'نسخه Manifest',children:selected.manifest_version||'—'},{key:'visibility',label:'نمایش در Catalog',children:selected.visibility_enabled?'فعال':'غیرفعال'}]}/>
          <Divider orientation="right">عملیات مجاز روی این منبع</Divider>{selected.source==='MANIFEST'&&!developmentMutationsEnabled&&<Alert type="info" showIcon message="فهرست عملیات از Manifest می‌آید؛ تخصیص دسترسی همچنان از بخش پایین انجام می‌شود." style={{marginBottom:12}}/>}<Row gutter={[10,10]}>{actions.map(action=>{const on=selected.actions.some((a:RowData)=>a.id===action.id);return <Col xs={12} md={8} key={action.id}><Card size="small"><Space><Switch size="small" disabled={selected.source==='MANIFEST'&&!developmentMutationsEnabled} checked={on} onChange={checked=>void toggleAction(action,checked)}/><span>{action.name_fa}</span><Typography.Text type="secondary">{action.action_key}</Typography.Text></Space></Card></Col>})}</Row>
        </Card>
    ...
    <Drawer width={560} open={editorOpen} title={editing?'ویرایش منبع':'تعریف منبع جدید'} onClose={()=>setEditorOpen(false)} extra={<Button type="primary" onClick={()=>form.submit()}>ذخیره</Button>}>
      <Form form={form} layout="vertical" onFinish={save} requiredMark="optional">{editing?.source==='MANIFEST'&&<Alert type="warning" showIcon message="مالک این منبع Manifest است" description={developmentMutationsEnabled?'تغییر توسعه‌ای موقت است و انتشار بعدی Manifest می‌تواند آن را بازنویسی کند.':'ساختار، metadata و عملیات فقط با publish یک Manifest معتبر تغییر می‌کنند؛ در این فرم فقط visibility قابل تغییر است.'} style={{marginBottom:16}}/>}<Form.Item name="type" label="نوع منبع" rules={[{required:true}]}><Select disabled={!!editing} optionRender={option=><Space><span>{typeMeta[String(option.value)]?.icon}</span><span>{option.label}</span></Space>} options={resourceTypes.map(type=>({value:type,label:typeMeta[type].label}))}/></Form.Item><Form.Item noStyle shouldUpdate>{()=>{const type=form.getFieldValue('type');return type&&<Alert type="info" showIcon message={typeMeta[type]?.label} description={typeMeta[type]?.hint} style={{marginBottom:16}}/>}}</Form.Item><Form.Item name="parentId" label="والد"><Select disabled={editing?.source==='MANIFEST'&&!developmentMutationsEnabled} allowClear showSearch optionFilterProp="label" options={resources.filter(r=>r.id!==editing?.id).map(r=>({value:r.id,label:`${r.name_fa} — ${r.resource_key}`}))}/></Form.Item><Form.Item name="resourceKey" label="کلید canonical" rules={[{required:true,message:'کلید یکتا الزامی است'},{pattern:/^[a-z][a-z0-9_.:/-]+$/,message:'فقط حروف کوچک انگلیسی، عدد و . _ : / -'}]} extra="پس از ایجاد برای منابع ADMIN و MANIFEST تغییرناپذیر است"><Input disabled={!!editing} placeholder="page:hr.employees"/></Form.Item><Row gutter={12}><Col span={12}><Form.Item name="nameFa" label="نام فارسی" rules={[{required:true}]}><Input disabled={editing?.source==='MANIFEST'&&!developmentMutationsEnabled}/></Form.Item></Col><Col span={12}><Form.Item name="nameEn" label="نام انگلیسی" rules={[{required:true}]}><Input disabled={editing?.source==='MANIFEST'&&!developmentMutationsEnabled}/></Form.Item></Col></Row><Row gutter={12}><Col span={12}><Form.Item name="ownerDomain" label="دامنه مالک"><Input disabled={editing?.source==='MANIFEST'&&!developmentMutationsEnabled} placeholder="hr"/></Form.Item></Col><Col span={12}><Form.Item name="classification" label="طبقه‌بندی"><Select disabled={editing?.source==='MANIFEST'&&!developmentMutationsEnabled} allowClear options={['PUBLIC','INTERNAL','CONFIDENTIAL','RESTRICTED'].map(value=>({value,label:value}))}/></Form.Item></Col></Row><Row gutter={12}><Col span={12}><Form.Item name="panelId" label="Micro Frontend مالک" extra={developmentMutationsEnabled?'در حالت توسعه ساخت دستی در MANIFEST mode مجاز است':'در MANIFEST mode ساخت دستی ممنوع است'}><Select disabled={!!editing} allowClear showSearch optionFilterProp="label" options={panels.map(panel=>({value:panel.id,label:`${panel.name_fa} — ${panel.resource_definition_mode}`}))}/></Form.Item></Col><Col span={12}><Form.Item name="source" label="منبع مالکیت"><Select disabled options={[{value:'ADMIN',label:'ADMIN'},{value:'MANIFEST',label:'MANIFEST'}]}/></Form.Item></Col></Row><Form.Item name="visibilityEnabled" valuePropName="checked"><Switch/> نمایش در Effective UI Catalog</Form.Item><Form.Item noStyle shouldUpdate>{()=>form.getFieldValue('type')==='EXTERNAL_RESOURCE'&&<Card size="small" title="شناسه منبع خارجی"><Form.Item name="externalSystem" label="سامانه"><Input disabled={editing?.source==='MANIFEST'&&!developmentMutationsEnabled} placeholder="superset"/></Form.Item><Row gutter={12}><Col span={12}><Form.Item name="externalType" label="نوع خارجی"><Input disabled={editing?.source==='MANIFEST'&&!developmentMutationsEnabled} placeholder="dashboard"/></Form.Item></Col><Col span={12}><Form.Item name="externalId" label="شناسه خارجی"><Input disabled={editing?.source==='MANIFEST'&&!developmentMutationsEnabled}/></Form.Item></Col></Row></Card>}</Form.Item></Form>
    </Drawer>
  </Space>
}
