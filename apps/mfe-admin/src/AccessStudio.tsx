...
export function AccessStudio(){
  ...
  const toggleAction=async(action:RowData,on:boolean)=>{if(!selected)return;await api(`/resources/${selected.id}/actions/${action.id}`,{method:on?'PUT':'DELETE'});await load();setSelected((await api('/resource-tree')).map((r:RowData)=>({...r,actions:JSON.parse(r.actions_json??'[]')})).find((r:RowData)=>r.id===selected.id))};
  const loadGrants=async(type:SubjectType,id:string)=>{setGrantLoading(true);try{setGrants(await api(`/subjects/${type}/${id}/grants`))}finally{setGrantLoading(false)}};
  ...
  const revoke=async(grantId:string)=>{await api(`/grants/${grantId}`,{method:'DELETE'});message.success('لغو دسترسی در صف همگام‌سازی OpenFGA قرار گرفت');if(subjectId)await loadGrants(subjectType,subjectId);await load()};
