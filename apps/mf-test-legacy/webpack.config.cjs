...
module.exports = {
  entry: './src/index.ts', output: { path: path.resolve(__dirname, 'dist'), publicPath: 'auto', uniqueName: 'aurevia_test_legacy', clean: true, filename: '[name].[contenthash].js', chunkFilename: '[name].[contenthash].js' },
  ...
  plugins: [new ResourceManifestWebpackPlugin('./resource-manifest.json'), new MicroFrontendManifestWebpackPlugin('./mf-manifest.json'),
    new HtmlWebpackPlugin({ title: 'Legacy Test Microfrontend' }),
    new ModuleFederationPlugin({ name: 'aurevia_test_legacy', filename: 'remoteEntry.js', exposes: { './plugin': './src/bootstrap.tsx' },
  ...
  devServer: { host: '127.0.0.1', port: 3012, historyApiFallback: true,
