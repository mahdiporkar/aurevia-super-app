...
function RemoteHost({panel,manifest,locale}:{panel:PanelManifest;manifest:EffectiveManifest;locale:Locale}){
  ...
  useEffect(()=>{let cleanup:(()=>void)|undefined,cancelled=false;setLoading(true);setError('');const scope=`aurevia_${panel.slug.replace(/^mfe-/,'').replaceAll('-','_')}`;loadRemote(scope,panel.remoteEntry,panel.exposedModule,manifest.panels.map(p=>p.remoteEntry),panel.integrity).then(remote=>{if(!cancelled&&host.current)cleanup=remote.mount(host.current,{locale,manifest,correlationId:()=>crypto.randomUUID()})}).catch(e=>{if(!cancelled)setError(e instanceof Error?e.message:String(e))}).finally(()=>{if(!cancelled)setLoading(false)});return()=>{cancelled=true;cleanup?.()}},[panel,manifest,locale]);
...
function App(){
  ...
  useEffect(()=>{
    ...
    fetch('/api/v1/me/manifest',{credentials:'same-origin',redirect:'manual',signal:controller.signal,headers:{'X-Correlation-ID':crypto.randomUUID()}})
      ...
      .catch(e=>{if(!(e instanceof Error&&(e.message==='AUTH_REDIRECT'||e.name==='AbortError')))setError(e instanceof Error?e.message:String(e))});
