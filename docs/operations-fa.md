...
2. `GET /api/me/context` برابر 200 و شامل `contractVersion: "1.0"` و `uiCatalog`؛ `/api/v1/me/manifest` فقط کنترل سازگاری است.
...
- سلامت BFF، instance انتخاب‌شده از Registry و policy مربوط به TLS/SSRF را بررسی کنید؛ Operation Gateway در flow Superset حضور ندارد.
...
خود deployment ثبت‌شدهٔ Superset باید assetهای static هم‌نسخه را سرو کند و مسیرهای سازگاری
BFF باید path را حفظ کنند. Aurevia Core دیگر مالک image عمومیِ asset-only نیست.
...
- DB/metadata Superset خارجی: dashboard/chart/dataset؛ lifecycle و recovery آن مستقل است.
- DB سرویس Identity Provider: هویت و client؛ Keycloak محلی اختیاری و خارج از Core است.
...
- مقصدهای Superset در production باید HTTPS، سازگار با outbound URL policy و دارای TLS/mTLS معتبر باشند.
- permission/RLS داخلی Superset و credentialهای DWH باید مستقل harden شوند.
