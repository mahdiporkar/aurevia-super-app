...
- Authorization Service آدرس Remote Entry، MF Manifest و Resource Manifest را مستقل و با origin
  فهرست دقیق URLهای همان catalog و HTTPS را validate می‌کند؛ این فهرست پویا با context کاربر
  ساخته می‌شود و origin allowlist استاتیک استقرار نیست. CSP لبه نیز مسیر same-origin را محدود می‌کند.
