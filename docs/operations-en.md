...
2. `GET /api/me/context` returns 200 and contains `contractVersion: "1.0"` plus `uiCatalog`; `/api/v1/me/manifest` remains a compatibility check only.
...
- Verify the BFF, the selected Registry instance, and its TLS/SSRF policy. Operation Gateway is not in the Superset flow.
...
Verify that the registered Superset deployment serves its own version-matched static assets and
that the BFF compatibility routes preserve their paths. Aurevia Core no longer owns a public
asset-only Superset image.
...
- External Superset metadata/database: dashboards, charts, datasets, and metadata; it follows that deployment's own lifecycle and recovery plan.
- Identity Provider database: identities and clients; the optional local Keycloak is not Core-owned.
...
- Every Superset Registry destination is HTTPS in production, passes the outbound URL policy, and uses the configured TLS/mTLS material.
- Native Superset permissions/RLS and DWH credentials remain independently hardened.
