...
اگر نشست ندارید، BFF شما را وارد Authorization Code Flow در Keycloak می‌کند. success handler فعلی پس از ورود به `/` بازمی‌گردد؛ در این حالت آدرس Swagger را یک‌بار دیگر باز کنید. سپس در dropdown بالای Swagger یکی از سه قرارداد را انتخاب کنید:
...
| `2 - API عمومی پنل راهبری` | pathهای واقعی `/api/v1/admin` که Admin MFE مصرف می‌کند؛ schemaها مستقیماً از کد سرویس مجوزدهی مشتق می‌شوند | `/api/v1/docs/admin/openapi` |
| `3 - سرویس مجوزدهی داخلی` | قرارداد داخلی کاربران، OU، گروه، نقش، منبع، route، Legacy، Superset، log و OpenFGA | `/api/v1/docs/authorization/openapi` |
...
قرارداد دوم projection خودکار عملیات `/internal/v1/registry/**` است: BFF پیشوند را به
`/api/v1/admin/**` تبدیل می‌کند، schema و operationId را بدون تغییر نگه می‌دارد و پارامترهای
`X-Actor*` را حذف می‌کند، چون این مقادیر فقط باید از نشست معتبر ساخته شوند. بنابراین این
قرارداد دقیقاً با URLهای مصرف‌شده در کد Admin MFE همگام است و snapshot دستی ندارد.
...
- JSON هر سه قرارداد بدون operation فاقد summary، schema یا پاسخ امنیتی است.
- مجموعهٔ method/path/operationId قرارداد Admin دقیقاً با projection عملیات Registry در
  قرارداد Authorization Service برابر است.
...
این فرمان با ورود واقعی، HTML و فایل‌های Swagger، JSON هر سه قرارداد، همگامی کامل قرارداد
Admin با Registry، شناسهٔ یکتای operation،
