...
| invalidation فقط tuple دقیق را حذف می‌کرد | P0 | `OpenFgaRelationshipAdapter` | graph epoch مشترک Redis قبل از هر graph mutation افزایش می‌یابد؛ شکست Redis mutation را متوقف می‌کند؛ check هنگام cache outage مستقیم OpenFGA را می‌خواند و برای جلوگیری از stale deny/allow با `HIGHER_CONSISTENCY` اجرا می‌شود | cache-hit و consistency option تست شده؛ replay واقعی V31 و allow/deny runtime کنترل شده است | Redis production HA |
...
| production profile fail-fast و ingress headers کامل نیست | P1 | Spring config، Nginx، Compose | OpenFGA در profile تولید برای Store/Model واقعی و HTTPS fail-fast شد؛ بخش ingress محیط‌محور باز است | تست guard تولید | دامنه، TLS و trusted proxy production |
