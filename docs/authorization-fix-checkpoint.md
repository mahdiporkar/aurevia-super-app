...
## Session 4 — Functional baseline (MFE + omnibus) COMPLETE (2026-09-20), uncommitted
Bugs: M1 compose hardcoded UI_ARTIFACT_* (env ignored); M2 generic MFE errors (DNS/TLS/connect/policy collapsed); M3 sync validated manifest's *declared* runtime against network/SRI; M4 localhost accepted at registration but refused at fetch in PROD/INTERNAL; F2 operations with methods the route never admits / config-time ambiguity; F4 grants for non-existent subjects; F5 demo contract_version "1"; T1 manual artifact publish failed (untyped NULL source_url); X1 test-sso fixture NPE on tokens without aud. Discovered: compose.e2e-auth-core.yml mounts target/*.jar → earlier e2e-auth 41/41 (session 3) ran Sep-18 jars; re-run on fresh jars: 41/41.
New: V73 (target auth profile optional, contract "1"→"1.0"), `tools/enterprise-hr-e2e.mjs` 16/16, `tools/mfe-external-e2e.mjs` 11/11, `UiArtifactNetworkMatrixTest` 54, Lifecycle +2 (Superset two-user/revoke, manual MFE), docs `system-functional-guide-fa.md`, `current-features-fa.md`, `microfrontend-registration-and-lifecycle-fa.md`.
Backend 412 run / 0 fail / 0 skip (authz 240, bff 108, ui-artifact-security 62, test-legacy 2); frontend 68.
