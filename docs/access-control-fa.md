...
## Context مؤثر رابط کاربری

`GET /api/me/context` قرارداد canonical شِل است و identity، `uiCatalog`، route، navigation،
منابع و فیلدهای سازگار manifest را یک‌جا برمی‌گرداند:
...
`GET /api/v1/me/manifest` به‌عنوان projection سازگاری باقی مانده است. UI می‌تواند دکمه یا
route را بر اساس context پنهان کند، اما پنهان‌کردن UI کنترل امنیتی نهایی نیست؛ API نیز باید
همان مجوز را enforce کند.
