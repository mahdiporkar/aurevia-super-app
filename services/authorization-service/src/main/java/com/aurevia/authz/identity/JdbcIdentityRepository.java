...
public class JdbcIdentityRepository implements IdentityRepository {
  ...
  @Override public Optional<RoleSnapshot> role(UUID id) {
    return database.sql("select role_key,version from application_role where id=:id for update")
  ...
  @Override public long upsertRoleAssignment(String type, UUID subjectId, UUID roleId,
    ...
    String status = lockRole(roleId);
    if (!"ACTIVE".equals(status)) {
      throw new IllegalArgumentException("Role must be active before assigning it");
    }
  ...
  @Override public void enqueueRoleAssignment(String type, UUID subjectId, UUID roleId,
    ...
    database.sql("""
        ...
        select 'role-assignment',r.id,:event,
          jsonb_build_object(
            'user',case :type when 'USER' then 'user:'||u.canonical_user_id
              when 'DIRECTORY_GROUP' then 'group:directory/'||dg.id||'#member'
              else 'group:'||lower(ag.code)||'#member' end,
            'relation','assignee','object','role:'||r.role_key),
          :event||':'||:type||':'||:subject||':'||:role||':'||:version||':'||cast(:mutation as text)
        ...
        .param("role", roleId).param("version", version)
        .param("mutation", UUID.randomUUID()).update();
  }

  private String lockRole(UUID roleId) {
    return database.sql("select status::text from application_role where id=:id for update")
        .param("id", roleId).query(String.class).optional()
        .orElseThrow(() -> new IllegalArgumentException("Role not found"));
  }

  @Override public void enqueueRoleStatus(UUID id, long version, boolean active) {
    // Use the same aggregate as ordinary assignment events, so a pending write can never
    // overtake the delete generated by deactivation. Keep assignments in SQL for reactivation.
    database.sql("""
        insert into outbox_event(aggregate_type,aggregate_id,event_type,payload,idempotency_key)
        select 'role-assignment',r.id,:event,
          jsonb_build_object('user',s.subject,'relation','assignee','object','role:'||r.role_key),
          'ROLE_STATUS:'||r.id||':'||cast(:version as text)||':'||s.subject
        from application_role r join (
          select x.role_id,'user:'||u.canonical_user_id subject,x.expires_at,u.status='ACTIVE' enabled
          from user_role_assignment x join app_user u on u.id=x.user_id
          union all
          select x.role_id,'group:directory/'||g.id||'#member',x.expires_at,g.status='ACTIVE'
          from group_role_assignment x join directory_group g on g.id=x.group_id
          union all
          select x.role_id,'group:'||lower(g.code)||'#member',x.expires_at,g.active
          from access_group_role_assignment x join access_group g on g.id=x.access_group_id
        ) s on s.role_id=r.id
        where r.id=:id and (not :active or
          (s.enabled and (s.expires_at is null or s.expires_at>now())))
        on conflict(idempotency_key) do nothing
        """).param("id", id).param("version", version).param("active", active)
        .param("event", active ? "ROLE_ASSIGNMENT_WRITE" : "ROLE_ASSIGNMENT_DELETE").update();
    database.sql("""
        insert into outbox_event(aggregate_type,aggregate_id,event_type,payload,idempotency_key)
        select 'grant',g.id,:event,
          ...
          'ROLE_STATUS_GRANT:'||ar.id||':'||cast(:version as text)||':'||g.id
        from authorization_grant g join application_role ar on ar.id=g.subject_id
        join resource r on r.id=g.resource_id
        where g.subject_type='ROLE' and ar.id=:id and g.status='ACTIVE'
          and (not :active or (r.status='ACTIVE' and (g.expires_at is null or g.expires_at>now())))
        on conflict(idempotency_key) do nothing
        """).param("id", id).param("version", version).param("active", active)
        .param("event", active ? "GRANT_WRITE" : "GRANT_DELETE").update();
  }
