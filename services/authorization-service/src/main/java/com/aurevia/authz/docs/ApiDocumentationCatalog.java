...
final class ApiDocumentationCatalog {
  ...
  private static Map<String, String> summaries() {
    ...
    put(m, "AuthorizationDiagnosticsController", "explain", "تشخیص علت مجاز یا غیرمجاز بودن دسترسی یک subject به یک منبع");
    ...
    put(m, "UiPluginRegistryController", "artifacts", "فهرست نسخه‌های Artifact میکروفرانت", "publish", "انتشار و اعتبارسنجی Artifact", "syncFrontendManifest", "همگام‌سازی مستقل MF Manifest", "activate", "فعال‌سازی نسخه Artifact", "menu", "ثبت override منوی قدیمی میکروفرانت", "navigationOverrides", "فهرست Navigation Overlayهای راهبر", "navigationDefinitions", "نمایش مقدار پیش‌فرض، override و مؤثر Navigation", "navigation", "ثبت Navigation Overlay یا گره ادمین", "removeNavigation", "حذف Navigation Overlay یا گره ادمین");
    put(m, "ResourceManifestController", "definition", "دریافت Manifest تعریف منابع یک برنامه", "stageLegacy", "تبدیل Manifest قدیمی به Draft بدون انتشار", "drafts", "فهرست revisionهای Manifest یک میکروفرانت", "fetch", "دریافت امن Manifest و ایجاد Draft", "importDraft", "اعتبارسنجی JSON و ایجاد Draft و Diff", "preview", "پیش‌نمایش Draft و تغییرات", "publish", "تأیید راهبر و انتشار اتمیک Resource Catalog", "activate", "فعال‌سازی هماهنگ نسخه منتشرشده منابع و رابط کاربری");
    ...
    put(m, "SupersetProxyResolutionController", "resolve", "Resolve نمونه عملیاتی Superset از نام عمومی", "resolveIntegration", "Resolve مقصد Superset از رجیستری پویا", "integrations", "فهرست Integrationهای Superset مجاز برای کاربر", "health", "ثبت وضعیت مستقل سلامت Superset");
