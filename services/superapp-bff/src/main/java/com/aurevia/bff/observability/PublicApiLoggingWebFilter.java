...
public class PublicApiLoggingWebFilter implements WebFilter {
  ...
  private Mono<Void> log(ServerWebExchange exchange,String correlation,long started,ByteArrayOutputStream body,Throwable failure) {
    return exchange.getPrincipal().map(java.security.Principal::getName).defaultIfEmpty("").flatMap(user->{
      Map<String,Object> value=entry(exchange,user,correlation,started,body,failure);
      if(failure instanceof org.springframework.web.server.ResponseStatusException status)
        value.put("statusCode",status.getStatusCode().value());
      for(String field:java.util.List.of("authorizationResult","resourceId","businessAction"))
        put(value,field,exchange.getAttribute("proxy."+field));
      return logs.ingestApiLog(value).onErrorResume(unavailable->Mono.empty());
    });
  }
