...
export async function verifySwaggerInChrome({origin,username,password}) {
  ...
  try {
    ...
    await read('(function(){const select=document.querySelector(".topbar select");select.value="/api/v1/docs/admin/openapi";select.dispatchEvent(new Event("change",{bubbles:true}));})()');
    await until('/عمومی راهبری/.test(window.ui?.specSelectors?.specJson()?.getIn(["info","title"])??"")','Public Admin contract did not load');
    steps.push(await execute('/api/v1/admin/panels','get'));
