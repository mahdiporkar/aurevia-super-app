...
const cases=['LOGIN-REDIRECT','JSON-UNAUTHENTICATED','SWAGGER-HTML','SWAGGER-ASSETS','SWAGGER-CONFIG',
  'BFF-CONTRACT','ADMIN-CONTRACT','AUTHORIZATION-CONTRACT','ADMIN-CONTRACT-SYNC','BFF-PUBLIC-LOGIN','BFF-PROXY-CSRF','IDENTITY-CONTRACT',
...
async function main() {
  ...
  await test('JSON-UNAUTHENTICATED','Authorization JSON façade returns 401 without session',async()=>{
    for(const path of ['/api/v1/docs/admin/openapi','/api/v1/docs/authorization/openapi']) {
      const r=await fetch(origin+path,{redirect:'manual'});assert.equal(r.status,401);
    }
    return 401;});
  ...
  await test('SWAGGER-CONFIG','Three same-origin contracts; no persisted authorization or external validator',async()=>{
    const r=await session.json('/v3/api-docs/swagger-config');assert.equal(r.status,200);
    assert.equal(r.body.persistAuthorization,false);assert.equal(r.body.urls.length,3);
  ...
  const contracts={};
  for(const [id,label,path] of [['BFF-CONTRACT','bff','/v3/api-docs'],['ADMIN-CONTRACT','admin','/api/v1/docs/admin/openapi'],['AUTHORIZATION-CONTRACT','authorizationService','/api/v1/docs/authorization/openapi']]) {
  ...
  assert(contracts.bff&&contracts.admin&&contracts.authorizationService,'Runtime specifications unavailable');
  await test('ADMIN-CONTRACT-SYNC','Every Authorization registry operation is projected to its real public BFF path',async()=>{
    const prefix='/internal/v1/registry';
    const expected=operations(contracts.authorizationService).filter(op=>op.path===prefix||op.path.startsWith(prefix+'/'))
      .map(op=>`${op.method} /api/v1/admin${op.path.slice(prefix.length)} ${op.operationId}`).sort();
    const actual=operations(contracts.admin).map(op=>`${op.method} ${op.path} ${op.operationId}`).sort();
    assert.deepEqual(actual,expected);assert.equal(contracts.admin.servers?.[0]?.url,'/');
    for(const op of operations(contracts.admin))for(const parameter of op.parameters??[])
      assert(!/^X-Actor(?:-Issuer|-Subject)?$/i.test(parameter.name),`Caller-supplied actor header: ${op.path}`);
    return {registryOperations:actual.length,pathAndOperationIdMismatches:0};});
  ...
  await test('READ-TRY-OUT','Read panels through the public Admin contract path',async()=>{
    const r=await session.json('/api/v1/admin/panels');assert.equal(r.status,200);assert(Array.isArray(r.body));return 200;});
